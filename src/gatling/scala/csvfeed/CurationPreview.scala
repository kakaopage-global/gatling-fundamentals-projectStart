...
import baseConfig.{IdnCurationApiSimulation}
...
class CurationPreview extends IdnCurationApiSimulation {
  ...
  def getPreview() = {
    exec(http("GET Curation content preview api")
      .get("api/curation/v1/content/14/preview")
      .header("user-id", "${id}")
      //      .queryParam("resourceType","ZIP")
      //      .queryParam("episodeId", ticketUseQueryMap("episodeId").toString())
    )
...
//  }
  ...
  val scn = scenario("Curation Gatling Test Scenario")
...
//        rampUsers(numberOfUsers).over(loadDuringSeconds seconds)
    )
